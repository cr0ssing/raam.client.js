...
function recreateMerkleTree(verificationKey, index, authPathHashes) {
    ...
    return producedHashes[producedHashes.length - 1]
}

function verifyMerkleTree(public, verificationKey, index, authPathHashes) {
    const merkleRoot = recreateMerkleTree(verificationKey, index, authPathHashes)
    return converter.trytes(public) == converter.trytes(merkleRoot)
}
